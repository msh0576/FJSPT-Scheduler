...
class TFJSPEnv(gym.Env):
    ...
    def step(self, action, step_reward=None):
        ...
        # self.feat_opes_batch[self.batch_idxes, ALLO_MA, ope] = ma.float()
        ...
        self.feat_vehs_batch[self.batch_idxes, 0, :] = torch.count_nonzero(self.ma_veh_adj_batch[self.batch_idxes, :, :], dim=1).float()  # the number of connected machines on a vehicle
